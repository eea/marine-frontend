pipeline {
  ...
  stage('Integration tests') {
     parallel {
      ...
      //            sh '''docker pull eeacms/volto-project-ci; docker run -i --name="$BUILD_TAG-cypress" --link $BUILD_TAG-plone:plone -e GIT_NAME=$GIT_NAME -e GIT_BRANCH="$BRANCH_NAME" -e GIT_CHANGE_ID="$CHANGE_ID" -e CYPRESS_PREFIX_PATH="/marine" eeacms/volto-project-ci cypress'''
      //          } finally {
      //            try {
